...
CREATE TABLE AUDITORIA (
    ...
    LLAVE_PRIMARIA VARCHAR(100), -- puede contener el valor de la PK afectada
    DETALLE TEXT -- opcional, para guardar JSON u otro detalle de los cambios
);

CREATE VIEW VENDEDORES_MENOS_VENTAS AS
SELECT 
    V.COD_VEN,
    CONCAT(V.NOM_VEN, ' ', V.APE_VEN) AS NOMBRE_COMPLETO,
    COUNT(F.NUM_FAC) AS TOTAL_VENTAS
FROM 
    VENDEDOR V
LEFT JOIN 
    FACTURA F ON V.COD_VEN = F.COD_VEN
GROUP BY 
